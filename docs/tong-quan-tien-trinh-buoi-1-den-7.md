# Tổng Quan Tiến Trình Phát Triển Hệ Thống CRS (Buổi 1 — Buổi 7)

Tài liệu ghi nhận toàn bộ lộ trình kiến trúc, các cột mốc tính năng và công nghệ đã triển khai từ Buổi 1 đến Buổi 7 trong dự án **CRS Microservices** (Course Registration System).
...
- **Kết quả**:
  ...
  - Component **`CourseForm` (Controlled Component)**: Dùng chung một form duy nhất cho cả hai luồng Thêm và Sửa thông qua `editingCourse` prop. Tự động đổ dữ liệu cũ khi bấm Sửa và làm sạch form khi Hủy/Thêm mới.
  - **Client-side Validation**: Kiểm tra rỗng và ràng buộc giá trị số dương (`soTinChi > 0`, `soChoToiDa > 0`) ngay tại trình duyệt, giảm tải request không hợp lệ lên server.
  - Bổ sung các API CRUD trong **`courseApi.ts`**: `createCourse`, `updateCourse`, `deleteCourse` chuẩn hóa payload kiểu số.
  - Cập nhật **`CourseList`**: Tích hợp các nút hành động "Sửa" và "Xoá" trên từng dòng bảng dữ liệu.
  - Ráp nối hoàn chỉnh tại **`App.tsx`**: Xử lý `extractErrorMessage` đọc cả 2 định dạng lỗi (`{ message: "..." }` và `{ tenMonHoc: "..." }`), đồng bộ lại danh sách (`refetch()`) sau khi thêm/sửa/xoá thành công mà không phải reload lại toàn trang.
...
│   ├── src/
│   │   ├── api/
│   │   │   ├── axiosClient.ts        # Axios client có Request Interceptor gắn Token
...
│   │   │   ├── CourseList.tsx        # Render bảng dữ liệu + nút Sửa/Xoá
│   │   │   └── CourseForm.tsx        # Form Controlled Component Thêm/Sửa + Client Validation
│   │   ├── types/
│   │   │   ├── apiError.ts
│   │   │   ├── auth.ts
│   │   │   ├── course.ts             # Course, CourseFormValues, emptyCourseForm, PagedResponse
│   │   │   └── registration.ts
│   │   ├── App.tsx                   # Ráp nối CourseForm, CourseList, Pagination, SearchBox
│   │   └── main.tsx
│   ├── .env
│   └── package.json
│
